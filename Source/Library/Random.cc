...
ParallelMT::ParallelMT(int threads, unsigned long long first_seed){
...
ParallelMT::Cycle(unsigned long long &init_key[], int key_length){
	
	for ( int i = 0; i < key_length; i++){
		
		if (init_key[i] < 18446744073709551615 - 2){
			// adding a small number results in maximal change
			init_key[i] += 2;
		
		} else {
			// cycle value around if beyond expected limit
			init_key[i] -= 18446744073709551615 - 2;
		}
	}
}
...
inline double ParallelMT::RandomReal(int thread) const {
	
	if (thread < _threads)
		return generator[thread] -> RandomReal();
	
	else throw IndexError("You requested a generator out of bounds!");
}
